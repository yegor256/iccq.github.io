...
<br/>
[Andrey Terekhov]():
PC Welcome Speech (not yet confirmed)
{: .agenda .agenda-10 .agenda-welcome}
...
Mutation Testing offers a powerful approach to assess- ing unit test set quality; however, software developers are often reluctant to embrace the technique because of the tremendous number of mutants it generates, in- cluding redundant and equivalent mutants. In spite of their large numbers, redundant mutants are relatively innocuous. Recent research indicates that redundant mutants affect a test engineer’s work effort only slightly, whereas equivalent mutants have a direct linear im- pact. Moreover, the time invested analyzing equivalent mutants produces no unit tests. Dominator mutants address the redundancy problem by allowing the tester to write a minimal number of tests. This paper demon- strates that mutant selection strategies do not matter, and that if equivalent mutants can be avoided, then an average of only 5.7% of mutants need to be analyzed to achieve mutation adequacy; otherwise, 65% of the mu- tants a tester evaluates on average will be equivalent. Apart from the negative impact of equivalent mutants, achieving mutation adequacy is relatively easy.
{: .abstract}
